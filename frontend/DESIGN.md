...
| Mid Gray | `#737373` | Muted body text, placeholder text, helper labels, icon fills at rest |
| Hairline | `#e5e5e5` | Borders, input outlines, card edges, badge outlines |
| Ember | `#e7000b` | Red decorative accent for icons, marks, and small graphic details. Use as a supporting accent, not as a status color |
...
- **Canvas** (`#f5f5f5`) — Page background, broadest layer
- **Sidebar** (`#fafafa`) — Navigation surface, one step lighter than canvas
- **Card** (`#ffffff`) — Primary content container, brightest surface
- **Input Fill** (`#f5f5f5`) — Resting input field, matches canvas tone for subtle differentiation
